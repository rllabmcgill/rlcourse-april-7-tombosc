...
from lstm import RLLSTM, ErrorPredictor
...
#ep = ErrorPredictor(feature_size, lr=0.01, beta=0.5)
...
for trial in range(args.n_trials):
    ...
    for i_episode in range(n_episodes):
        ...
        while not done and t < limit:
            a_t, A_t = agent.sample_action(feature_func(o_t))
            o_t_1, r_t, done, info = env.step(a_t)
            #observations.append(feature_func(o_t_1))
            r_t = reward_map[r_t] # to make rewards only at the end of episode
            r_buf += r_t
